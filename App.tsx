...
const App: React.FC = () => {
    ...
    return (
        <div className={`h-screen w-screen flex flex-col md:flex-row overflow-hidden bg-black text-white ${isRTL() ? 'rtl' : ''}`} style={{ filter: grayscaleMode ? 'grayscale(100%)' : 'none' }}>
            <Sidebar
                currentView={view}
                                    ...
                                    {isLoading ? (
                                        ...
                                        <>
                                            ...
                                            {(activeSearchCategory === 'ALL' || activeSearchCategory === 'ALBUMS') && (
                                                <MediaGrid title={t('albums')} items={resultAlbums} type="album" />
                                            )}

                                            {(activeSearchCategory === 'ALL' || activeSearchCategory === 'ARTISTS') && (
                                                <MediaGrid title={t('artists')} items={resultArtists} type="artist" />
                                            )}

                                            {(activeSearchCategory === 'ALL' || activeSearchCategory === 'PLAYLISTS') && (
                                                <MediaGrid title={t('playlists')} items={resultPlaylists} type="playlist" />
                                            )}
                                        </>
                                    ) : (
                                        // Recent Searches
                                        searchHistory.length > 0 && (
                                            <div className="animate-fade-in">
                                                <h2 className="text-xl font-bold mb-4">{t('recent_searches')}</h2>
            ...
            {showSettingsModal && (
                <SettingsModal
                    ...
                    language={language} setLanguage={(l) => { storageService.setLanguage(l); setLanguage(l); refreshLibrary(); }}
                />
            )}
