...
import { Check, Wifi, Palette, Sliders, Timer, ListCollapse, EyeOff, Square, Monitor, Gauge, Droplets, Type, Keyboard, Activity, Languages } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
    ...
    const tabs = [
        { id: 'QUALITY', label: t('audio_quality'), icon: Wifi },
        { id: 'APPEARANCE', label: t('appearance'), icon: Palette },
        { id: 'PLAYBACK', label: t('playback'), icon: Timer },
        { id: 'TWEAKS', label: t('tweaks'), icon: Sliders },
    ];
            ...
            case 'APPEARANCE':
                return (
                    <div className="animate-slide-up">
                        <div className="flex flex-col gap-4 mb-8">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <button
                                    ...
                                    className={`py-3 rounded font-medium text-sm transition-colors border ${language === 'EN' ? 'bg-white text-black border-white' : 'bg-[#3e3e3e] text-white border-transparent hover:bg-[#4e4e4e]'}`}
                                >
                                    {t('english')}
                                </button>
                                <button
                                    onClick={() => setLanguage('AR')}
                                    className={`py-3 rounded font-medium text-sm transition-colors border ${language === 'AR' ? 'bg-[#1db954] text-white border-[#1db954]' : 'bg-[#3e3e3e] text-white border-transparent hover:bg-[#4e4e4e]'}`}
                                >
                                    {t('arabic')}
                                </button>
                            </div>
                        ...
                        <h4 className="font-bold mb-2">{t('style_options')}</h4>
                        <div className="flex flex-col gap-2">
                            <ToggleItem
                                label={t('show_visualizer')}
                    ...
                    <div className="flex flex-col gap-6 animate-slide-up">
                        ...
                        <div>
                            <div className="flex items-center gap-2 font-bold mb-3">
                                <Timer size={18} /> <span>{t('sleep_timer')}</span>
                            </div>
                            ...
                            <div className="mt-4 flex gap-2">
                                ...
                                />
                                <Button size="sm" onClick={handleCustomTimer} disabled={!customTimer}>{t('set')}</Button>
                            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-scale-in">
            <div className="bg-[#181818] rounded-xl shadow-2xl w-full max-w-2xl h-[90vh] md:h-[80vh] flex flex-col md:flex-row overflow-hidden border border-[#282828]">
                ...
                <div className="w-full md:w-1/3 bg-[#121212] border-b md:border-b-0 md:border-r border-[#282828] p-4 flex md:flex-col gap-2 overflow-x-auto md:overflow-visible flex-shrink-0">
                    <h2 className="text-xl font-bold px-4 mb-2 md:mb-4 hidden md:block">{t('settings')}</h2>
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
